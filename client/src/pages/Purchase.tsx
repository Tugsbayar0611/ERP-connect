...
export default function Purchase() {
  ...
  const getDefaultWarehouseId = () => warehouses.find((warehouse: any) => warehouse.isDefault)?.id || warehouses[0]?.id || "";

  useEffect(() => {
    const warehouseId = getDefaultWarehouseId();
    if (warehouseId && !form.getValues("warehouseId")) {
            ...
            {isLoading ? (
              ...
              filteredOrders.map((order) => (
                <TableRow key={order.id} className="transition-colors">
                  ...
                  <TableCell className="text-muted-foreground">{order.orderDate ? format(new Date(order.orderDate), "yyyy-MM-dd") : "-"}</TableCell>
                  <TableCell className="text-muted-foreground">{order.expectedDate ? format(new Date(order.expectedDate), "yyyy-MM-dd") : "-"}</TableCell>
                  <TableCell>{getStatusBadge(order.status)}</TableCell>
                  <TableCell className="text-right font-semibold text-emerald-600 dark:text-emerald-400">{formatMNT(order.totalAmount || "0")}</TableCell>
                  <TableCell>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input placeholder="Тэмдэглэл..." {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                                  ...
                                  <Select
                                    onValueChange={(value) => {
                                      ...
                                      if (product && currentPrice === 0) {
                                        form.setValue(`lines.${index}.unitPrice`, Number(product.costPrice || 0));
                                      }
                                    }}
                                    value={field.value}
                                  >
