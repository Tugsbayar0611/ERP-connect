      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {isLoadingStats ? (
          ...
          departmentsWithStats.map((dept: DepartmentWithStats) => {
            ...
            const managerInitials = hasManager 
              ? `${dept.manager?.firstName?.[0] || ""}${dept.manager?.lastName?.[0] || ""}`.toUpperCase() 
              : "?";

            return (
              <Card
                key={dept.id}
                className="group relative overflow-hidden flex flex-col justify-between border-slate-200 dark:border-slate-800 transition-all duration-300 hover:shadow-xl hover:shadow-primary/5 hover:-translate-y-1 bg-white dark:bg-slate-950"
              >
                {/* Decorative Gradient Background */}
                <div className="absolute inset-0 bg-gradient-to-br from-primary/5 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none" />
                
                <CardHeader className="pb-4 border-b border-slate-100 dark:border-slate-800/50 relative z-10 pt-5">
                  <div className="flex justify-between items-start mb-3">
                    <div className="p-2.5 bg-gradient-to-br from-primary/10 to-primary/5 rounded-xl border border-primary/10 shadow-sm group-hover:scale-110 transition-transform duration-300">
                      <Building2 className="w-5 h-5 text-primary" />
                    </div>
                    {dept.code && (
                      <Badge variant="secondary" className="px-2.5 py-0.5 text-[11px] font-medium tracking-wider uppercase bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-400">
                        {dept.code}
                      </Badge>
                    )}
                  </div>
                  <CardTitle 
                    className="text-lg font-bold tracking-tight cursor-pointer group-hover:text-primary transition-colors line-clamp-1"
                ...
                <CardContent className="pt-5 pb-5 space-y-5 relative z-10 flex-1 flex flex-col">
                  {/* Grid Stats */}
                  <div className="grid grid-cols-2 gap-3 shrink-0">
                    <div className="flex flex-col p-3 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-800/50">
                      <div className="flex items-center gap-1.5 text-muted-foreground mb-1">
                        <Users className="w-3.5 h-3.5" />
                        <span className="text-xs font-medium uppercase tracking-wider">Нийт ажилтан</span>
                      </div>
                      <span className="text-xl font-bold text-foreground">{dept.employeeCount}</span>
                    </div>

                    <div className="flex flex-col p-3 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-800/50">
                      <div className="flex items-center gap-1.5 text-muted-foreground mb-1">
                        <CalendarCheck className="w-3.5 h-3.5" />
                        <span className="text-xs font-medium uppercase tracking-wider">Өнөөдрийн ирц</span>
                      </div>
                      <span className={`text-xl font-bold ${
                        dept.attendanceKPI >= 80 ? "text-emerald-600 dark:text-emerald-500" : 
                        dept.attendanceKPI >= 60 ? "text-amber-600 dark:text-amber-500" : 
                  ...
                  <div className="flex items-center gap-3 shrink-0">
                    <Avatar className={`h-10 w-10 border-2 border-white dark:border-slate-950 shadow-sm ${!hasManager && "opacity-50 grayscale"}`}>
                      {hasManager ? (
                  ...
                  {/* Spacer to push employees stack & button to bottom */}
                  <div className="flex-1" />

                  {/* Bottom section: Avatars & Add Button */}
                  <div className="flex items-center justify-between pt-1 shrink-0">
                        ...
                        <div className="flex -space-x-2.5">
                          {dept.topEmployees.slice(0, 4).map((emp, idx) => {
                            ...
                            return (
                              <Avatar
                                key={emp.id}
                                className="h-8 w-8 border-[2px] border-white dark:border-slate-950 ring-1 ring-slate-100 dark:ring-slate-800 shadow-sm"
                                style={{ zIndex: 10 - idx }}
                              >
                                <AvatarFallback className="text-[10px] bg-slate-50 dark:bg-slate-800 text-slate-600 dark:text-slate-300 font-semibold cursor-default hover:bg-primary/10 hover:text-primary transition-colors">
                                  {initials}
                                </AvatarFallback>
                              </Avatar>
                            );
                          ...
                          {dept.employeeCount > dept.topEmployees.length && (
                            <div className="h-8 w-8 rounded-full border-[2px] border-white dark:border-slate-950 bg-slate-100 dark:bg-slate-800 flex items-center justify-center font-medium text-[10px] text-slate-500 ring-1 ring-slate-100 dark:ring-slate-800 z-0">
                              +{dept.employeeCount - dept.topEmployees.length}
                            </div>
                          )}
                        </div>
                      ) : (
                        <span className="text-xs text-muted-foreground italic">Ажилтан байхгүй</span>
                      )}
                    </div>
                    ...
                    {isAdmin && (
                      <Button
                        variant="secondary"
                        size="sm"
                        className="h-8 px-3 rounded-full text-xs font-semibold shadow-none border border-slate-200 dark:border-slate-800 hover:bg-primary/5 hover:text-primary hover:border-primary/20 transition-all gap-1.5"
                  ...
                  <div className="absolute top-4 right-4 z-20 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                    <DropdownMenu>
                      <DropdownMenuTrigger asChild>
                        <Button variant="ghost" size="icon" className="h-8 w-8 rounded-full bg-white/80 dark:bg-slate-900/80 backdrop-blur-sm border border-slate-200 dark:border-slate-800 shadow-sm hover:bg-slate-50 dark:hover:bg-slate-800">
                          <MoreHorizontal className="w-4 h-4 text-slate-600 dark:text-slate-300" />
                        </Button>
                      </DropdownMenuTrigger>
                      <DropdownMenuContent align="end" className="w-40 rounded-xl">
                        <DropdownMenuItem onClick={() => openDialog(dept)} className="cursor-pointer gap-2 py-2">
                          <Pencil className="h-4 w-4 text-slate-500" />
                          <span className="font-medium">Засах</span>
                        </DropdownMenuItem>
                        <DropdownMenuItem
                          onClick={() => handleDelete(dept)}
                          className="cursor-pointer gap-2 py-2 text-rose-600 focus:text-rose-600 focus:bg-rose-50 dark:focus:bg-rose-500/10"
                        >
                          <Trash2 className="h-4 w-4" />
                          <span className="font-medium">Устгах</span>
                        </DropdownMenuItem>
                      </DropdownMenuContent>
                    </DropdownMenu>
                  </div>
