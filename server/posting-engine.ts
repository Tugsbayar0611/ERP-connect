...
export async function previewPosting(
  ...
  if (!journal) {
    // Lazy create default journal if missing
    ...
    const name = nameMap[journalType] || `${journalType.charAt(0).toUpperCase() + journalType.slice(1)} Journal`;
    const code = codeMap[journalType] || journalType.substring(0, 3).toUpperCase();

    try {
      [journal] = await db.insert(journals).values({
        tenantId,
        name,
        code,
        type: journalType,
      ...
      console.log(`Auto-created missing journal: ${name} (${code})`);
    } catch (err) {
      // Fallback if creation fails (e.g. duplicate code constraint) - try to find by code?
      // For now just re-throw original error context
      throw new Error(`Journal with type '${journalType}' not found and could not be created: ${err}`);
    }
  }
